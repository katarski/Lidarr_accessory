...
class Orchestrator:
    ...
    _SETTINGS_SCHEMA = [
        ...
        ("lidarr.interactive_search_artist_level", "lidarr", "interactive_search_artist_level", "Discography fallback", "bool", True,
         "When an album cannot be found on its own, search the whole ARTIST and accept a discography or box set that contains it (auto-deselect then drops everything already owned). Off = only ever search for the exact album."),
        ("lidarr.interactive_search_cooldown_seconds", "lidarr", "interactive_search_cooldown_seconds", "Retry cooldown (s)", "int", 43200,
         "How long before the same album is searched again, and before the same artist's discography scope is swept again. Stops a pass re-running the searches that just failed."),
        ...
        ("lidarr.interactive_search_min_title_ratio", "lidarr", "interactive_search_min_title_ratio", "Min title match", "float", 0.55,
         "How closely a release title must resemble the album (0-1). Lower finds more, at the risk of grabbing a different record; the song-title check below is the real safety net."),
        ("lidarr.interactive_search_seeder_weight", "lidarr", "interactive_search_seeder_weight", "Swarm-health weight", "float", 1.0,
        ...
        ("qbittorrent.stalled_reaper", "qbittorrent", "stalled_reaper", "Stalled-download reaper", "bool", True,
         "A partly-downloaded torrent with no progress for N days: salvage any finished songs a monitored album or an assembly needs (torrent removed, files kept), otherwise delete the torrent AND its data."),
        ("qbittorrent.stalled_grace_days", "qbittorrent", "stalled_grace_days", "Stalled grace (days)", "int", 3,
         "How many days without progress before a partly-downloaded torrent is salvaged/trashed."),
        ...
        ("lidarr.external_audit_interval_seconds", "lidarr", "external_audit_interval_seconds", "MusicBrainz: interval (s)", "int", 21600,
         "Seconds between MusicBrainz cross-check passes."),
        ("lidarr.external_audit_artists_per_pass", "lidarr", "external_audit_artists_per_pass", "MusicBrainz: artists / pass", "int", 10,
         "How many artists to cross-check per pass (MusicBrainz allows one request a second, so this bounds the pass length)."),
        ("lidarr.external_audit_recheck_seconds", "lidarr", "external_audit_recheck_seconds", "MusicBrainz: recheck (s)", "int", 604800,
         "How long before the same artist is cross-checked again. Default is a week."),
        ("lidarr.external_audit_include_eps", "lidarr", "external_audit_include_eps", "MusicBrainz: include EPs", "bool", False,
         "Report EPs and singles as well as studio albums."),
        ("lidarr.min_match_percent", "lidarr", "min_match_percent", "Min track match (%)", "float", 60.0,
         "How much of an album's tracks a download must supply before it is treated as that album."),
        ("lidarr.verify_track_titles_accept", "lidarr", "verify_track_titles_accept", "Song-title accept (0-1)", "float", 0.6,
         "Song-title coverage at or above this accepts the release outright."),
        ("lidarr.verify_track_titles_reject", "lidarr", "verify_track_titles_reject", "Song-title reject (0-1)", "float", 0.25,
         "Song-title coverage below this rejects the release as a different record, even when the track count matches."),
        ("lidarr.pre_check_library", "lidarr", "pre_check_library", "Check library first", "bool", True,
         "Ask Lidarr whether it already owns the album before doing any work on the download."),
        ("lidarr.force_import_on_count_match", "lidarr", "force_import_on_count_match", "Force-import on count match", "bool", True,
         "When Lidarr's own matcher refuses but the track count matches a monitored album, import against that album explicitly."),
        ("lidarr.force_import_max_missing_percent", "lidarr", "force_import_max_missing_percent", "Force-import: max missing (%)", "int", 10,
         "How much of the album may be absent and still be force-imported."),
        ("lidarr.force_import_partial", "lidarr", "force_import_partial", "Force-import partial albums", "bool", True,
         "Allow a force-import that fills only part of an album."),
        ("lidarr.force_import_partial_min_percent", "lidarr", "force_import_partial_min_percent", "Force-import: min partial (%)", "int", 50,
         "A partial force-import must supply at least this much of the album."),
        ("lidarr.force_import_max_extra_percent", "lidarr", "force_import_max_extra_percent", "Force-import: max extra (%)", "int", 25,
         "How many files beyond the album's track count are tolerated (bonus tracks, extras)."),
        ("lidarr.strict_import_only", "lidarr", "strict_import_only", "Strict import only", "bool", False,
         "Never force-import: if Lidarr's matcher declines, leave the album for you to handle by hand."),
        ("lidarr.content_identify_min_coverage", "lidarr", "content_identify_min_coverage", "Content-identify: min coverage (%)", "int", 60,
         "How many of an album's track titles must be recognised in the download before it is imported under Lidarr's title."),
        ("lidarr.reconcile_import_mode", "lidarr", "reconcile_import_mode", "Reconcile import mode", "str", 'copy',
         "'copy' leaves the download in place (safer, needs the space); 'move' relocates the files into the library."),
        ("lidarr.reconcile_max_probes_per_pass", "lidarr", "reconcile_max_probes_per_pass", "Reconcile: probes / pass", "int", 60,
         "How many candidate folders a reconcile pass may inspect. Bounds how long a pass takes on a large downloads folder."),
        ("lidarr.reconcile_recheck_seconds", "lidarr", "reconcile_recheck_seconds", "Reconcile: recheck (s)", "int", 21600,
         "How long before the same folder is examined again after it failed to import."),
        ("lidarr.verify_library_after_import", "lidarr", "verify_library_after_import", "Verify after import", "bool", True,
         "Confirm the tracks really landed in the library before deleting anything from the download."),
        ("staging.delete_originals_on_success", "staging", "delete_originals_on_success", "Delete originals after import", "bool", True,
         "Delete the individual source files once a verified import has copied them into the library."),
        ("lidarr.cleanup_lidarr_queue", "lidarr", "cleanup_lidarr_queue", "Clean Lidarr's queue", "bool", True,
         "Remove this download's rows from Lidarr's queue once it is imported."),
        ("lidarr.manual_import_timeout_seconds", "lidarr", "manual_import_timeout_seconds", "Import timeout (s)", "int", 300,
         "How long to wait for Lidarr's manual-import command to finish before giving up on it."),
        ("lidarr.wait_for_lidarr", "lidarr", "wait_for_lidarr", "Wait for Lidarr", "bool", True,
         "At startup, wait until Lidarr answers instead of failing the first passes."),
        ("watch.stable_seconds", "watch", "stable_seconds", "Settle time (s)", "int", None,
         "How long a file must stop changing size before it is considered finished downloading."),
        ("staging.album_folder_template", "staging", "album_folder_template", "Album folder template", "str", '{album} ({year})',
         "Folder name for staged albums. Placeholders: {album} {artist} {year}."),
        ("staging.filename_template", "staging", "filename_template", "Filename template", "str", '{artist} - {album} - {number:02d} - {title}.{ext}',
         "Track filename. Placeholders: {artist} {album} {number} {title} {ext}."),
        ("staging.mode", "staging", "mode", "Staging mode", "str", 'in_place',
         "'in_place' converts and imports where the download sits; 'copy' works on a copy in the staging folder."),
        ("lidarr.queue_reaper_enabled", "lidarr", "queue_reaper_enabled", "Legacy queue reaper", "bool", False,
         "Time-based removal of stuck Lidarr queue rows. Superseded by the disk-aware completed-torrent lifecycle, which runs instead when it is on."),
        ("lidarr.queue_reaper_interval_seconds", "lidarr", "queue_reaper_interval_seconds", "Queue reaper: interval (s)", "int", 600,
         "Seconds between legacy queue-reaper passes."),
        ("lidarr.queue_reaper_grace_minutes", "lidarr", "queue_reaper_grace_minutes", "Queue reaper: grace (min)", "int", 30,
         "How long a queue row must sit stuck before the legacy reaper removes it."),
        ("lidarr.queue_reaper_remove_from_client", "lidarr", "queue_reaper_remove_from_client", "Queue reaper: remove torrent too", "bool", True,
         "Also delete the torrent from qBittorrent, not just the Lidarr queue row."),
        ("lidarr.queue_reaper_blocklist", "lidarr", "queue_reaper_blocklist", "Queue reaper: blocklist", "bool", False,
         "Blocklist the release so Lidarr searches for a different one instead of re-grabbing it."),
        ("lidarr.held_auto_resolve_max_per_pass", "lidarr", "held_auto_resolve_max_per_pass", "Auto-resolve: max / pass", "int", 6,
         "How many needs-attention rows may be settled automatically per pass."),
        ("lidarr.assembly_interval_seconds", "lidarr", "assembly_interval_seconds", "Assembly: interval (s)", "int", 1800,
         "Seconds between assembly planning passes."),
        ("lidarr.assembly_hunt_per_pass", "lidarr", "assembly_hunt_per_pass", "Assembly: hunts / pass", "int", 2,
         "How many assemblies may search torrents for their missing songs per pass."),
        ("lidarr.assembly_max_albums_per_pass", "lidarr", "assembly_max_albums_per_pass", "Assembly: albums / pass", "int", 150,
         "How many missing albums to plan per pass."),
        ("lidarr.assembly_min_score", "lidarr", "assembly_min_score", "Assembly: min song score (0-1)", "float", 0.87,
         "How closely a source song's title must match a wanted track to count as that track."),
        ("lidarr.assembly_require_artist", "lidarr", "assembly_require_artist", "Assembly: require artist match", "bool", True,
        ...
        ("lidarr.transcode_dsd", "lidarr", "transcode_dsd", "Transcode DSD", "bool", True,
         "Convert SACD DSF/DFF to FLAC."),
        ("lidarr.transcode_dts_cd", "lidarr", "transcode_dts_cd", "Transcode DTS-CD", "bool", True,
         "Decode DTS-CD (DTS inside WAV) to multichannel FLAC."),
        ("ffmpeg.flac_compression_level", "ffmpeg", "flac_compression_level", "FLAC compression (0-12)", "int", None,
         "Higher is smaller and slower. 8 is the usual choice."),
        ("lidarr.tag_identify_pre_split", "lidarr", "tag_identify_pre_split", "Identify before splitting", "bool", True,
         "Work out which album a cue/image is before splitting it, so the tracks are named correctly the first time."),
        ("lidarr.pre_split_monitored_gap_only", "lidarr", "pre_split_monitored_gap_only", "Split only monitored gaps", "bool", True,
         "Only split a cue image when it fills an album Lidarr actually wants."),
        ("staging.delete_cue_if_pre_split", "staging", "delete_cue_if_pre_split", "Delete cue after split", "bool", True,
         "Remove the .cue once its image has been split into tracks."),
        ("watch.sweep_cueless_pre_split", "watch", "sweep_cueless_pre_split", "Sweep folders without a cue", "bool", False,
         "Also process download folders that have no .cue (loose tracks, ISOs, archives)."),
        ("watch.sweep_interval_seconds", "watch", "sweep_interval_seconds", "Sweep interval (s)", "int", 0,
         "Seconds between sweeps of the downloads folder. 0 = only at startup."),
        ("watch.sweep_min_stable_seconds", "watch", "sweep_min_stable_seconds", "Sweep: min stable (s)", "int", 300,
         "How long a folder must be unchanged before a sweep touches it."),
        ("watch.sweep_ledger_enabled", "watch", "sweep_ledger_enabled", "Remember swept folders", "bool", True,
        ...
        ("lidarr.library_audit_on_startup", "lidarr", "library_audit_on_startup", "Library audit at startup", "bool", False,
         "Run the audit once when the container starts."),
        ("lidarr.library_audit_interval_seconds", "lidarr", "library_audit_interval_seconds", "Library audit interval (s)", "int", 0,
         "Seconds between library audits. 0 = never on a timer."),
        ("lidarr.library_audit_skip_unchanged", "lidarr", "library_audit_skip_unchanged", "Library audit: skip unchanged", "bool", True,
         "Skip album folders whose contents have not changed since the last audit."),
    ]

    # Recommended values for the per-section 'Recommended' button. The
    ...
    _SETTINGS_RECOMMENDED = {
        ...
        "lidarr.interactive_search_min_missing_days": 3,
        "lidarr.interactive_search_max_albums_per_pass": 25,
        "lidarr.interactive_search_interval_seconds": 3600,
        "lidarr.interactive_search_cooldown_seconds": 43200,
        "lidarr.interactive_search_artist_level": True,
        ...
        "lidarr.strict_import_only": False,
    }
    ...
    _SETTINGS_GROUPS = [
        ("Finding missing music", [
            ...
            "lidarr.interactive_search_artist_level",
            "lidarr.interactive_search_cooldown_seconds",
            "lidarr.external_audit_enabled",
            "lidarr.external_audit_interval_seconds",
            "lidarr.external_audit_artists_per_pass",
            "lidarr.external_audit_recheck_seconds",
            "lidarr.external_audit_include_eps",
        ]),
        ("Which release to accept", [
            ...
            "lidarr.interactive_search_max_candidates",
            "lidarr.interactive_search_min_title_ratio",
            "lidarr.min_match_percent",
            "lidarr.verify_track_titles_accept",
            "lidarr.verify_track_titles_reject",
        ]),
        ("Importing", [
            ...
            "lidarr.pre_check_library",
            "lidarr.force_import_on_count_match",
            "lidarr.force_import_max_missing_percent",
            "lidarr.force_import_partial",
            "lidarr.force_import_partial_min_percent",
            "lidarr.force_import_max_extra_percent",
            "lidarr.strict_import_only",
            "lidarr.content_identify_min_coverage",
            "lidarr.reconcile_import_mode",
            "lidarr.reconcile_max_probes_per_pass",
            "lidarr.reconcile_recheck_seconds",
            "lidarr.verify_library_after_import",
            "staging.delete_originals_on_success",
            "lidarr.cleanup_lidarr_queue",
            "lidarr.manual_import_timeout_seconds",
            "lidarr.wait_for_lidarr",
            "watch.stable_seconds",
            "staging.album_folder_template",
            "staging.filename_template",
            "staging.mode",
        ]),
        ("Torrents", [
            ...
            "lidarr.queue_reaper_enabled",
            "lidarr.queue_reaper_interval_seconds",
            "lidarr.queue_reaper_grace_minutes",
            "lidarr.queue_reaper_remove_from_client",
            "lidarr.queue_reaper_blocklist",
        ]),
        ("Needs attention & Assembly", [
            ...
            "lidarr.held_auto_resolve_max_per_pass",
            "lidarr.assembly_interval_seconds",
            "lidarr.assembly_max_albums_per_pass",
            "lidarr.assembly_hunt_per_pass",
        ...
        ("Discs, archives & conversion", [
            ...
            "lidarr.transcode_lossless_to_flac",
            "lidarr.transcode_dsd",
            "lidarr.transcode_dts_cd",
            "ffmpeg.flac_compression_level",
            "lidarr.tag_identify_pre_split",
            "lidarr.pre_split_monitored_gap_only",
            "staging.delete_cue_if_pre_split",
        ]),
        ("Library & sweeps", [
            "watch.sweep_cueless_pre_split",
            "watch.sweep_interval_seconds",
            ...
            "lidarr.library_audit_skip_unchanged",
        ]),
    ]
